...
- `src/shared/app-logo.ts` now supplies an un-wired local-only logo customization core: shipped presets, bounded PNG validation, crop/fit/focal/background transforms, derived-only persistence, and stable identity safeguards. Settings and packaged rendering still need to consume it.
- `src/shared/file-converter.ts` now supplies an un-wired truthful conversion catalog and bounded persistent queue. Every adapter remains visibly unavailable until a bundled offline adapter, output validation, and packaged proof are added.
