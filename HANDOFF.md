...
- The local Ollama destination uses only the fixed documented loopback API boundary. It provides an accessible Model Store search, health and recovery state, conservative evidence-only fit cards, a bounded non-payment pull cart, local chat, and allowlisted harness planning. A reviewed official catalogue source, hardware probe, native attachment/export handoff, executable launch/rollback, and packaged interaction proof remain fail-closed boundaries.
- `v0.1.10501` supplied eight decoded hidden-desktop package captures for the app-logo studio, converter catalog/queue, and Ollama status/store/cart/chat/harness states. `docs/screenshots/smoke/metadata.json` binds each frame to commit `4a7db224f2ff43b21978627281bf68264abeb5bf`, the validated Squirrel full package, and zero visible-desktop or system-changing actions.
- `v0.1.10701` supplied a decoded hidden-desktop Ollama hardware-evidence capture bound in the current `docs/screenshots/smoke/metadata.json` to commit `9131e57f86a8ec09d81bc6595021f0f807d66c19`, a validated Squirrel package, and zero visible-desktop or system-changing actions.
